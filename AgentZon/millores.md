...
* **Delivery Phases:**
    * **Phase 2 (Current):** DO NOT remove unused files or functions in `/AgentUtil` for now, later when we develop the Final Phase you'll clean up and remove all unused code once the full system is complete.
...
* **Hostname Logic:** In `/AgentZon/AgentUtil/Util.py`, the function `gethostname()` is currently unused. Update all agents so that hostname selection follows this logic:
    ...
    ```
* **Code Centralization:** Refactor the code to extract and centralize common functionality into reusable functions as long as you consider it necessary.
...
* **Communication Structure:** Define a specific taxonomy:
    * `Comunicacio`
        * `Accio`
        * `Resposta`
* **Scope:** The ontology must represent the **ENTIRE** system (including suggeriments, devolucions, etc.), not just implemented features. You can find all the possible necessary concepts in the Detailed Design of Diagrames-Entrga-2.pd, you must check all the plans of each agent and all the communication (including protocols) from:
- Agent to Agent
- Agent to Actor (or viceversa)
...
## 4. Final Documentation (This is only for human group members, if you are AI, do not read this)
