...
**Comanda**
Atributs: `IdComanda`, `IdUsuari`, `Nom`, `Carrer`, `Ciutat`, `Prioritat`, `MetodePagament`. Relacions: `TeProducte` → `Producte`.
...
**Bases de dades de suport**
`HistorialCompra`, `HistorialCerca`, `DadesEnviamentUsuari`, `DadesBancaries` i `UbicacionsProductes` són fonts de dades persistides, no classes de l'ontologia. Quan una dada d'aquestes bases participa en una comunicació, s'expressa amb els conceptes del domini o del missatge corresponent, per exemple `Comanda`, `PeticioCerca`, `PeticioRegistreCompra` o la relació directa `UbicatACentre` entre `Producte` i `CentreLogistic`.
